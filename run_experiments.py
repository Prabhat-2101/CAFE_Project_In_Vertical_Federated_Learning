...
def main():
    ...
    for num_clients in clients_list:
        for alpha in alphas_list:
            ...
            # 5. Extract Metrics
            losses_centralized = history.losses_centralized or []
            acc_tuples = history.metrics_centralized.get('accuracy', []) if history.metrics_centralized else []
            
            rounds = [l[0] for l in losses_centralized]
            losses = [l[1] for l in losses_centralized]
            accuracies = [m[1] for m in acc_tuples]
            
            if not rounds:
                rounds = [i+1 for i in range(num_rounds)]
                losses = [0] * num_rounds
            if not accuracies:
                accuracies = [0] * len(rounds)
